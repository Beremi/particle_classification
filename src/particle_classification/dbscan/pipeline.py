...
from .reference import dbscan_labels, dbscan_labels_pairs
...
from ..data.t3pa import FTOA_SUBTICKS_PER_TOA, TOA_TICK_NS, count_t3pa_rows, iter_t3pa_hits
...
def load_t3pa_hit_arrays(
    ...
    *,
    ...
    include_special_records: bool = False,
) -> T3PAHitArrays:
    """Load a `.t3pa` file into typed NumPy arrays.

    When `max_hits` is smaller than the file row count, a deterministic
    contiguous raw-row span is loaded. This keeps local hit density meaningful
    for DBSCAN tuning while avoiding multi-million-row tuning passes.

    T3PA rows with a nonzero ``Overflow`` field are device markers (for
    example, matrix indices 116/117 delimit a data-loss interval), not detector
    hits. They are excluded by default and counted in
    ``special_records_skipped``. Set ``include_special_records=True`` only for
    raw-format diagnostics; :func:`iter_t3pa_hits` always remains lossless.
    """
    ...
    for file_row, hit in enumerate(iter_t3pa_hits(source)):
        ...
        if file_row >= stop_row:
            break
        if hit.is_special_record and not include_special_records:
            special_records_skipped += 1
            continue
        x[filled] = hit.x
        y[filled] = hit.y
        toa[filled] = hit.toa
    ...
    return T3PAHitArrays(
        ...
        special_records_skipped=special_records_skipped,
    )
...
def write_particle_npz(
    ...
    *,
    ...
    payload = {
        ...
        "source_special_records_skipped": np.asarray(arrays.special_records_skipped, dtype=np.int64),
    ...
    return {
        ...
        "special_records_skipped": arrays.special_records_skipped,
    }
